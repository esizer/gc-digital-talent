...
const useServerResponsiveTableController = <TData extends object, TFilters>({
  data,
  columns,
  hiddenColumnIds,
  isLoading,
  ...
  const pageCount =
    pagination && !pagination.internal
      ? Math.ceil(
          (pagination.total ?? 0) /
            (state.pagination?.pageSize ?? INITIAL_STATE.paginationState.pageSize),
        )
      : undefined;
  ...
  return {
    ...
    paginationAdjusted: pagination,
    rowSelectionCount: Object.values(rowSelection).length,
  };
};

export default useServerResponsiveTableController;
